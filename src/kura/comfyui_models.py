"""ComfyUI workflow model discovery and Hugging Face registry resolution."""
...
def resolve_model_specs(workflow: dict[str, Any], registry: Any) -> tuple[list[dict[str, Any]], list[dict[str, str]]]:
    ...
    for ref in required_model_refs(workflow):
        ...
        if not isinstance(repo, str) or not repo or not isinstance(filename, str) or not filename:
            unknown.append(ref)
            continue
        target_dir = COMFYUI_MODEL_DIRS.get(ref["type"], ref["type"])
        spec = {
            **ref,
            "repo": repo,
            "filename": filename,
