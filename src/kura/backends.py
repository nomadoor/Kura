...
def compile_ai_toolkit(run: dict[str, Any], destination: Path) -> None:
    ...
    if isinstance(native, dict):
        ...
        return
    lines = [
        "# Generated by kura; expand this adapter for full AI-Toolkit support.",
        "[model]",
        f"base = {json.dumps(model.get('base', ''))}",
        "[datasets]",
        "ids = " + json.dumps([dataset.get("id", "") for dataset in datasets]),
        "[training]",
    ]
    for key in ("rank", "alpha", "lr", "scheduler", "steps", "batch_size", "resolution", "seed"):
...
def command_ai_toolkit(run: dict[str, Any]) -> dict[str, Any]:
    ...
    if command is None and isinstance(run.get("backend_overrides", {}).get("ai-toolkit", {}).get("config"), dict):
