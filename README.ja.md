...
- **sd-scripts** は、監査済みの upstream release に固定した Kura 管理イメージで動きます。Kuraは、SD 1.5、SDXL、FLUX.1、Anima LoRA、Anima ControlNet-LLLiteについて、ローカル環境で実際に optimizer step まで通しています。正確な検証範囲は[backend対応表](docs/backend-support.md)を参照してください。これは実行互換性の確認であり、あらゆるdatasetや設定でのLoRA品質を保証するものではありません。
