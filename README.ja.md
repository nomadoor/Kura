...
Kura は「学習ソフト」そのものではありません。AI-Toolkit や Musubi Tuner といった学習ツールを、**安全に・記録を残しながら・AIから扱いやすく**するための、薄い管理レイヤーです。
...
- **Windows / WSL2 の人**：Docker Desktop の設定で対象ディストリの WSL integration をオンにしてください。Docker のディスクイメージとキャッシュは空き容量の大きいドライブに置くと安心です。
...
- 回収後は既定で `--hold-for 30m` だけ Pod を残し、LoRAを確認できる
- その後、特別な指示が無ければ自動で `terminate` される
